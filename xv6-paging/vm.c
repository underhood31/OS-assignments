...
select_a_victim(pde_t *pgdir)
{
  ...
  for (int i = PGSIZE+1; i < size; i+=PGSIZE)
  {
    // cprintf("select_a_victim\n");
    pte_t *curEntry;
    curEntry = uva2pte(pgdir,i);
    if (curEntry!=0)
    {
      if (!(*curEntry&PTE_A) && (*curEntry&PTE_U) && !(*curEntry&PTE_SWP1))
      {
        
        uint dst;
        // cprintf("Returning address: %d\n", *curEntry);
        //invalidate cache entry
       asm ("INVLPG %1"
         : "=r" (dst)
         : "r" (i));
        // cprintf("Invalidating caching done\n");
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if (!(*pte & PTE_SWP1))
    {
      ...
      //add swapped page location to the page directory.
      ...
      read_page_from_disk(curproc->cwd->dev, (void*)mem, bid);
      
      if(mappages(d, (void*)i, PGSIZE, V2P(mem), PTE_P|PTE_U|PTE_W) < 0)
        goto bad; 
    }
