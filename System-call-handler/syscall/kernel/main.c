...
static void register_syscall(void)
{
	...
	printk("new_table pointer: %llu\n",(unsigned long long)new_table);
	printk("new_table base: %llu\n",(unsigned long long)new_table->base);
	bits=bits>>16;
	no128_syscall = new_table->base + 128;
	my_system_call = new_table->base + 15;
	printk("size of first en %llu\n",(unsigned long long)my_system_call);
	printk("15 pointer: %llu\n",(unsigned long long)my_system_call);
	printk("128 pointer: %llu\n",(unsigned long long)no128_syscall);
